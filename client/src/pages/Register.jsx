...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!result.success) {
      ...
      if (role === 'organizer') {
        toast.info('Account ready! Awaiting admin approval.');
      } else {
  ...
  return (
    <div className="min-h-screen pt-20 pb-10 flex flex-col items-center justify-center bg-[#0B0F19] relative z-10 selection:bg-pink-500/30">
      ...
      <div className="w-full max-w-[400px] px-6 animate-fade-in flex flex-col items-center">
        ...
        {/* Action Card */}
        <div className="w-full bg-[#151D2A]/80 backdrop-blur-xl rounded-[2rem] p-8 shadow-2xl border border-white/5 relative overflow-hidden text-slate-300">
          ...
          <form onSubmit={handleRegister} className="space-y-5 relative z-10">
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 ml-1 text-left">Full Name</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 ml-1 text-left">Email Address</label>
              <input
                type="email"
                required
                className="w-full bg-[#0F1520] border border-white/5 rounded-xl px-4 py-3 text-white focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all placeholder:text-slate-600"
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 ml-1 text-left">Password</label>
              <input
                type="password"
                required
                className="w-full bg-[#0F1520] border border-white/5 rounded-xl px-4 py-3 text-white focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-all placeholder:text-slate-600"
          ...
          <div className="relative my-8 z-10">
            ...
            <div className="relative flex justify-center text-xs uppercase tracking-widest">
              <span className="bg-[#151D2A] px-4 text-slate-500 font-bold">Or Continue With</span>
            </div>
          </div>
          ...
          {role === 'organizer' && (
             <p className="text-[10px] text-pink-400/60 mt-6 text-center font-medium uppercase tracking-wider relative z-10">
        ...
        <p className="text-center mt-8 text-sm text-slate-500 font-medium">
          Joined us before? <Link to="/login" className="text-pink-400 hover:text-pink-300 font-bold tracking-wide transition-colors">Log In</Link>
        </p>
